...
<?php include('facture_getinfos.php'); ?>
<html>
<head>
<title>WATS - Facture#<?php echo $_GET['id_facture']; ?></title>
...
<body>
	...
	<p style="right: 0; position: absolute; color: gray; text-align: right; margin-top: 20px; margin-right: 10px">
		<span style="font-size: 14px; font-weight: bold;">FACTURE</span>
		<br>
		<?php echo $FactureInfos['numero_facture']; ?>
		<br><br>
		<span style="font-size: 14px; font-weight: bold;">N° CLIENT</span>
		<br>
		<?php echo $FactureInfos['id_client']; ?>
		<br>
		<br>
		<span style="font-size: 14px; font-weight: bold;">DATE</span>
		<br>
		<?php echo TransformDateFR($FactureInfos['date_facture']); ?>
		<br>
	</p>
	...
	<?php if($FactureInfos['statut'] == "1") { ?>
	<p>
		<b>Mode de réglement :</b> <?php echo $FactureInfos['type_reglement']; ?><br>
		<b>Date du paiement :</b> <?php echo TransformDateFR($FactureInfos['date_paiement']); ?><br>
	</p>
	<?php } ?>
	<p>
		<b>Echéance :</b> <?php echo $ClientInfos['echeance_paiement']; ?>
	</p>
	<!-- --------------------- -->
	...
	<table class="designations-table">
		...
		<?php foreach($FactureDesigs as $presta) { ?>
		<tr>
...
<div class="garder_samepage">
	...
	<table class="tableau2">
		<tr>
			...
			<td style="text-align: right;"><?php echo FormatEuro($FactureInfos['total_ht']); ?></td>
		</tr>
		<tr>
			<td style="width:40%; text-align: left; border: none">NET HT</td>
			<td style="text-align: right; border: none"><?php echo FormatEuro($FactureInfos['total_ht']); ?></td>
		</tr>
		<tr>
			<td style="width:40%; text-align: left;">TVA</td>
			<td style="text-align: right;">
						<?php
								if($FactureInfos['tva_applicable'] == 1) {
										echo FormatEuro(ConvertTTC($FactureInfos['total_ht']) - $FactureInfos['total_ht']);
								} else {
										echo FormatEuro(0);
								}
						?>
		...
		<tr>
			...
			<td style="text-align: right; border: none"><?php echo FormatEuro(ConvertTTC($FactureInfos['total_ht'], $FactureInfos['tva_applicable'])); ?></td>
		</tr>
		<tr>
			<td style="width:40%; text-align: left; font-weight: bold; border: none; color: #FF4961;">NET À PAYER</td>
			<td style="text-align: right; border: none; color: #FF4961;"><?php echo FormatEuro(ConvertTTC($FactureInfos['total_ht'], $FactureInfos['tva_applicable'])); ?></td>
		</tr>
		<tr>
	...
	<table class="tableau3">
		...
		<tr>
			<td><?php echo FormatEuro($FactureInfos['total_ht']); ?></td>
			<td>
					<?php if($FactureInfos['tva_applicable'] == 1) {
									echo "20%";
								} else {
						 			echo "0%";
								}
			...
			<td>
						<?php
									if($FactureInfos['tva_applicable'] == 1) {
												...
												echo FormatEuro($FactureInfos['total_ht']);
									}
						?>
			</td>
		</tr>
	...
	<p style="right: 0; position: absolute; text-align: center; font-size: 10px; margin-right: 15px">
		<?php if($FactureInfos['tva_applicable'] == 0) { echo "TVA non applicable. <br>"; } ?>
